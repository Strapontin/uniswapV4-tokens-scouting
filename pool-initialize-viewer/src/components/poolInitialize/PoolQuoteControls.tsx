...
function decimalStringToBaseUnits(value: string, decimals: number): bigint {
  ...
  const safeFractionalPart = fractionalPart.replace(/[^0-9]/g, "").slice(0, decimals);
  const normalized = `${integerPart}${safeFractionalPart.padEnd(decimals, "0")}`;
  const result = BigInt(normalized);

  return negative ? -result : result;
}

function amountToDecimalString(value: string, decimals: number): string {
  const trimmed = value.trim();
  if (trimmed === "") {
    return "0";
  }
  ...
  try {
    const normalizedAmount = trimmed.includes(".") ? decimalStringToBaseUnits(trimmed, decimals) : BigInt(trimmed);
    return formatUnits(normalizedAmount, decimals);
  } catch {
    return "0";
  }
}
...
export function PoolQuoteControls({ pools, defaultTokenIn }: PoolQuoteControlsProps) {
  ...
  const decimalConvertedAmountIn = amountToDecimalString(amountIn, getMetadataDecimals(normalizeAddress(tokenIn)));
  ...
  const handleConvertAmount = async () => {
    try {
      ...
      const decimals = getMetadataDecimals(normalizedTokenIn);
      const rawAmount = decimalStringToBaseUnits(amountIn, decimals);
      setAmountIn(rawAmount.toString());
      setError(null);
    } catch (conversionError) {
      setError(conversionError instanceof Error ? conversionError.message : "Failed to convert amount.");
    }
  };
        ...
        <div className="flex flex-wrap items-end gap-3">
          ...
          <label className="flex min-w-0 flex-1 flex-col gap-1">
            ...
            <span className="mt-1 font-mono text-[10px] uppercase tracking-[.08em] text-[#8d9b93]">
              Decimal value: <span className="text-[#edf4ef]">{decimalConvertedAmountIn}</span> {selectedToken?.symbol ?? ""}
            </span>
          </label>
